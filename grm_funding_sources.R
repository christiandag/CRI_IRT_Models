### Funding sources construct analysis using GRM model ###
...
# Select the columns corresponding to Funding Sources items
ddd<-ddd[,9:13]
...
mod.pcm<-mirt(ddd, model=1, itemtype="graded", SE=T)   #3.1_39

# Check the model fit
M2(mod.pcm, type="C2") #3.1_52

# Estimate the latent trait of the national multisectoral sample
latent_trait_all <- fscores(mod.pcm, method="EAP", full.scores=T, full.scores.SE = T)
...
latent_trait_tech <- fscores(mod.pcm, method="EAP", full.scores=T, full.scores.SE = T, response.pattern=tech[,9:13])
...
hist(latent_trait_tech, main = "Funding sources", ylab = "Frequency", xlab="Latent trait", 
